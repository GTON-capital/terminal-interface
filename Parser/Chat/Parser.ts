...
import { fantomRpc, gtonMainnetAddress } from '../../config/config-test';
// Func Router
...
function getUserByArg(list: ListItem[], arg: string): ListItem {
  ...
  const i = list.findIndex(
    (e) => e.name.toLowerCase() === lowerArg || e.address.toLowerCase() === lowerArg.substring(2),
  );
...
const angelWorker = ({ print }) => {
  print([
    textLine({
      words: [
        textWord({
          characters: `To participate in angel program complete the steps below
...
const sendWorker = createWorker(async ({ print }, args, [userAddress]) => {
  ...
  if (type === SendArgs.dm) {
    ...
    if (whitelist.length === 0)
      throw new Error('User does not have enough gton to receive message');
  } else {
...
const loadWorker = createWorker(async ({ print }, args, [userAddress]) => {
  ...
  if (type === SendArgs.dm) {
    ...
    res = await makeRequest(Routes.Get, {
      address: sender,
      chat_name: chatName,
      address_from: receiver.address,
      limit,
    });
  } else {
  ...
  if (res.length > 0) {
    ...
    res.map(async (val) => {
      ...
      print([
        textLine({
          words: [
            textWord({ characters: `Sender -  ${val.name} (Address - 0x${val.from_address})` }),
          ],
        }),
      ]);
